...
class ParameterSpec(BaseModel):
    ...
    `type` is a plain `str`, not a closed enum. Review-time definitions may
    declare types this tool does not special-case (`array`, `object`, anything
    else). An unrecognised type must route through the string path downstream
    rather than fail validation and kill the whole run. A missing `type`
    defaults to `"string"` for the same reason.
    """
...
class CallResult(BaseModel):
    ...
    Serialising this model with `model_dump` yields precisely the three keys the
    output contract (subject V.4) requires, no extras.
    """
